...
public class EmployeeUpdateDto {
    ...
    @NotBlankAndPattern(
            ...
            pattern = "^[가-힣]+$",
            groups = NameValidationGroup.class
    )
    ...
    @Pattern(regexp = "^010\\d{4}\\d{4}$|^$", message = "휴대폰 번호 형식을 확인해주세요.", groups = DefaultGroup.class)
    private String empPhone;
    private String empSign;

    @NotBlank(message = "우편번호를 확인해주세요.", groups = DefaultGroup.class)
    @Size(min = 5, max = 5, message = "우편번호는 정확히 5자리여야 합니다.", groups = DefaultGroup.class)
    private String empZip;

    @NotBlank(message = "도로명 주소를 입력하세요.", groups = DefaultGroup.class)
